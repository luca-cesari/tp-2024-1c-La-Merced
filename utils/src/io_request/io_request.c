...
t_io_request *crear_io_request(u_int32_t pid, char *nombre_interfaz, char *instruccion, char *argumentos)
{
   ...
   io_request->pid = pid;
   io_request->interface_name = strdup(nombre_interfaz);
...
t_packet *serializar_io_request(t_io_request *io_request)
{
   ...
   agregar_a_paquete(paquete, io_request->interface_name, strlen(io_request->interface_name) + 1);
...
{
   free(io_request->interface_name);
   free(io_request->instruction);
   free(io_request->arguments);

   io_request->interface_name = strdup("");
...
t_io_request *recibir_io_request(int32_t fd_conexion)
{
   ...
   io_request->interface_name = strdup(list_get(paquete, 1));
